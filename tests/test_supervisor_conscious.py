"""Tests for SupervisorConscious (conscious mind)."""
...
def _make_supervisor(tmp_path, sex="male"):
    engine = EmotionEngine()
    stm = ShortTermMemory()
    ltm = LongTermMemory(path=tmp_path / "ltm.json")
    s = SupervisorConscious(sex=sex, emotion_engine=engine, stm=stm, ltm=ltm, config=_make_config())
    return s, engine, stm, ltm
...
class TestParseToolCalls:
    def test_single_tool_call(self):
        ...
        calls = SupervisorConscious._parse_tool_calls(text)
        assert len(calls) == 1
...
class TestExtractResponse:
    def test_simple_response(self):
        ...
        resp = SupervisorConscious._extract_response(text)
...
class TestExecTool:
    def test_stm_store(self, tmp_path):
        s, _, stm, _ = _make_supervisor(tmp_path)
        result = s._exec_tool({"tool": "stm_store", "content": "test memory"})
    ...
    def test_stm_recall(self, tmp_path):
        ...
        stm.store("item1")
        stm.store("item2")
        result = s._exec_tool({"tool": "stm_recall", "n": 5})
        assert "item1" in result
        assert "item2" in result

    def test_ltm_store(self, tmp_path):
        s, _, _, ltm = _make_supervisor(tmp_path)
        result = s._exec_tool({
            "tool": "ltm_store",
    ...
    def test_ltm_search(self, tmp_path):
        ...
        result = s._exec_tool({"tool": "ltm_search", "query": "python"})
        assert "Python" in result

    def test_emotion_adjust(self, tmp_path):
        s, engine, _, _ = _make_supervisor(tmp_path)
        result = s._exec_tool({"tool": "emotion_adjust", "emotion": "joy", "amount": 0.2})
    ...
    def test_emotion_adjust_clamps(self, tmp_path):
        ...
        s._exec_tool({"tool": "emotion_adjust", "emotion": "joy", "amount": 0.9})
        # Should be clamped to 0.3
        snap = engine.snapshot()
        assert snap["joy"] <= 0.31  # Allow tiny floating point margin

    def test_think(self, tmp_path):
        s, _, _, _ = _make_supervisor(tmp_path)
        result = s._exec_tool({"tool": "think", "thought": "Let me reason..."})
        assert "thought noted" in result.lower()

    def test_unknown_tool(self, tmp_path):
        s, _, _, _ = _make_supervisor(tmp_path)
        result = s._exec_tool({"tool": "nonexistent"})
...
class TestBuildSystem:
    def test_includes_profile(self, tmp_path):
        s, _, _, _ = _make_supervisor(tmp_path, sex="male")
        system = s._build_system()
    ...
    def test_female_profile(self, tmp_path):
        s, _, _, _ = _make_supervisor(tmp_path, sex="female")
        system = s._build_system()
...
class TestProcess:
    ...
    def test_direct_response(self, mock_chat, tmp_path):
        ...
        s, _, stm, _ = _make_supervisor(tmp_path)
        result = s.process("A bird flies by")
    ...
    def test_with_tool_use_then_response(self, mock_chat, tmp_path):
        ...
        s, _, _, _ = _make_supervisor(tmp_path)
        result = s.process("What do you think?")
    ...
    def test_with_reflex_result(self, mock_chat, tmp_path):
        ...
        result = s.process("Loud bang!", reflex_result)
    ...
    def test_max_rounds_fallback(self, mock_chat, tmp_path):
        ...
        s, _, _, _ = _make_supervisor(tmp_path)
        result = s.process("Complex problem")
    ...
    def test_history_trimming(self, mock_chat, tmp_path):
        ...
        s._history = [{"role": "user", "content": f"msg-{i}"} for i in range(25)]

        s.process("new stimulus")
        # After process, history should be trimmed
        assert len(s._history) <= 22  # 20 old + 2 new (user + assistant)
